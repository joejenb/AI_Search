...
def main():
	best_tours = []
	best_tour, climbed = initialise(), True
	while climbed and time.time()-start_time < 110:
		new_tour = climb(best_tour)
		if new_tour == best_tour:
			best_tours.append(best_tour)
			best_tour = initialise()
		else:
			best_tour = new_tour
			best_tours.append(best_tour)	
	best_tour = max(best_tours, key=lambda x: x[1])
	return  best_tour[0], best_tour[1]
...
def climb(best_tour):
	...
	for i in range(num_cities-2):
		for j in range(i+1, num_cities-2):
			new_tour = best_tour[0][:i+1]+best_tour[0][i+1:j+1][::-1]+best_tour[0][j+1:]
			tour_length = calc_length(new_tour)
			if tour_length > best_tour[1]:
				best_tour = [new_tour, tour_length]
			if flipper:
...
all_tours = set()
threshold, start_time = sum(max(x) for x in distance_matrix), time.time()
tour, tour_length_thresh = main()
tour_length = threshold - tour_length_thresh
#######################################################################################################

...
def main():
	cities, population = [i for i in range(num_cities)], []
	max_f, peak_time = (None, 0), 1
	for i in range(pop_size):
		mem = random.sample(cities, num_cities)
		population.append((mem, fitness(mem)))# Try list pairs
	while time.time() -start_time < 115:
		total = sum(x for _, x in population)
		population, peak_time, max_f = evolve(population, peak_time, max_f, total)
	return max_f[0], threshold - max_f[1] 


def evolve(population, peak_time, max_f, total):
	new_pop, normalised = [], [x/total for _, x in population]# Try mapping
	for i in range(pop_size):
		...
		new_pop.append(z)
	peak_time += 1
	return new_pop, peak_time, max_f


def fitness(mem):
	length = 0
	for i in range(len(mem)-1):
		length += distance_matrix[mem[i]][mem[i+1]]
	length += distance_matrix[mem[-1]][mem[0]]
	return threshold - length


def reproduce(x, y):
	split = random.randint(0, num_cities)
	xy = x[0][:split] + y[0][split:]
	yx = y[0][:split] + x[0][split:]
	dup_xy = [i for i in range(num_cities) if xy[i] in xy[:i]]# Try using enumerate
	dup_yx = [i for i in range(num_cities) if yx[i] in yx[:i]]#Try using non in line for loop
	for i, dup_i in enumerate(dup_xy):
		xy[dup_i], yx[dup_yx[i]] = yx[dup_yx[i]], xy[dup_i]
	fit_xy, fit_yx = fitness(xy), fitness(yx)
...
def mutate(tour):
	if random.randrange(1000) < mut_percentage:
		s = random.sample(range(num_cities), 2)
		tour[s[0]], tour[s[1]] = tour[s[1]], tour[s[0]]
	return tour
